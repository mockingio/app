<template>
  <div class="flex-1 flex items-center">
    <div class="flex-1" v-if="activeMock">
      ...
      <p class="text-xs text-gray-500 dark:text-slate-600">
    ...
    <Popover class="relative flex">
      <PopoverButton
        class="text-white items-center text-base font-medium focus:outline-none"
      >
        <DotsVerticalIcon class="text-gray-900 dark:text-slate-500 w-6 h-6" />
      </PopoverButton>
      <PopoverPanel
        v-slot="{ close }"
        class="absolute z-10 right-[10px] top-[15px] transform mt-3 px-2 w-64"
      >
        <div

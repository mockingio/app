<template>
  <router-link
    ...
    :to="{
      name: 'route-view',
      params: { routeId: route.id, id: mock.data.id },
    }"
  >
    <div
      :class="[
        isActive ? 'bg-slate-800' : 'border-transparent',
        'hover:bg-slate-800 flex items-center group block text-sm py-1',
      ]"
    >
      <div class="pl-3 pr-1 py-1 flex-1">
        <span class="flex items-center">
          <span
            :class="`rounded p-1 text-white w-[40px] method-${route.method.toLowerCase()} text-xxs`"
            >{{ shortMethod }}</span
          >

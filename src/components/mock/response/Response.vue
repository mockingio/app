<template>
  <div>
    <div
      @click="toggleOpen"
      class="select-none flex dark:bg-slate-800 bg-white cursor-pointer handle group"
    >
      <div class="m-3 block flex-1 flex justify-between">
        <div>{{ status }}</div>
        <TrashIcon
          class="w-5 h-5 ml-5 hover:text-red-500 invisible group-hover:visible"
        />
      </div>
    </div>
              ...
              >
                <button
                  :class="[
                    ...
                    'focus:outline-none whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm hover:border-green-600',
                  ]"
                >
                  {{ item.name }}
                </button>
...
const status = computed(
  () => getStatusById(props.response.status.toString())?.name
);

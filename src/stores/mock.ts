...
export const useMockStore = defineStore({
  ...
  getters: {
    getMockByID(state) {
      return (id: string): Mock | undefined =>
        state.mocks.find((m) => m.data.id === id);
    },
    activeRoute(state) {
  ...
  actions: {
    ...
    async patchRoute(
      ...
      this.mocks[mockIdx] = {
        ...mock,
        data: { ...mock.data, routes: [...mock.data.routes] },
      };
      // send data to server
    ...
    async patchResponse(
      ...
      const responseIdx = mock.data.routes[routeIdx].responses.findIndex(
        (r) => r.id === responseId
      );
      ...
      mock.data.routes[routeIdx].responses[responseIdx] = {
        ...mock.data.routes[routeIdx].responses[responseIdx],
        ...data,
      };
      ...
      // send data to server
      return axios.patch(
        getUrl(`/mocks/${mockId}/routes/${routeId}/responses/${responseId}`),
        data
      );
    },
